module.exports = function(grunt) {
  ...
  var dirsConfig = {
      ...
      server:'.',
      bowerComponents:'./public/lib',
      distBowerComponents:'./dist/public/lib'
    };
  ...
  grunt.initConfig({
    ...
    jshint: {
      all: ['<%= dirs.client %>/**/*.js','<%= dirs.client %>/**/**/*.js'] 
    },
    ...
    injector: {
      ...
      local_dependencies: {
        files: {
          '<%= dirs.distClient %>/index.html': ['<%= dirs.distClient %>/**/*.js','<%= dirs.distClient %>/**/**/*.js', '<%= dirs.distClient %>/*.css']
        }
      }
    },
    ...
    uglify: {
      app: {
        files: {
          '<%= dirs.distClient %>/application.js': ['<%= dirs.distClient %>/**/*.js','<%= dirs.distClient %>/**/**/*.js']
        }
      }
    },
    ...
    ngAnnotate: {
        dist: {
            files: [{
                    expand: true,
                    src: ['<%= dirs.client %>/**/*.js','<%= dirs.client %>/**/**/*.js', '!/<%= dirs.client %>**/*.annotated.js'],
                    ext: '.js',
                    extDot: 'last'
                }],
        }
    ...
    html2js: {
      options: {
        ...
        main: {
          src: ['<%= dirs.distClient %>/**/views/*.html','!<%= dirs.distClient %>/index.html'],
    ...
    nodemon: {
      dev: {
        script: '<%= dirs.server %>/application.js'
      }
    }   
